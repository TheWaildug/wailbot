...
client.on("voiceStateUpdate", (oldState, newState) => {
  ...
  if (newState.channelID === generalchannel.id) {
    ...
    if (!newState.member.roles.cache.some(role => role.name === "General"))
      newState.member.roles.add(role); // Add the role to the user if they don't already have it
  }
  ...
  if (command === "membercount") {
    ...
    message.channel.send(
      "The current member count is " +
        `${message.guild.memberCount.toLocaleString()}`
    );
    ...
    message.member.send(
      "Invite the bot to your server! https://discord.com/oauth2/authorize?client_id=755781017889144903&scope=bot&permissions=8"
    );
    return;
  }
});
